...
export default function BookingPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("success");
      setMessage("Your appointment has been confirmed. You will receive an email confirmation shortly.");
  ...
  return (
    <main className="page">
      <section className="header">
        ...
        <p>
          Choose a date and time that suits you, and meet with one of our jewellery experts
          at our Leicester showroom.
        </p>
      </section>
      ...
      <form className="booking" onSubmit={handleSubmit}>
        <section className="panel">
          ...
          <div className="cards">
            {consultations.map((item) => (
              <button
                type="button"
                className={item.id === consultationId ? "card active" : "card"}
                key={item.id}
        ...
        <section className="panel">
          ...
          <div className="grid">
            ...
            <label>
              Contact Number *
              <input name="phone" required placeholder="Enter mobile number" />
            </label>

            <label>
              Address *
              <input name="address" required placeholder="Enter address" />
            </label>
          </div>

          <label>
            Notes
            <textarea name="notes" rows={4} placeholder="Tell us anything helpful before your visit"></textarea>
          </label>
        </section>
        ...
        <section className="panel">
          ...
          <div className="grid">
            ...
            <label>
              ...
              <select name="time" required disabled={!date || slotLoading || slots.length === 0}>
                <option value="">
                  {slotLoading ? "Checking availability..." : "Select a time"}
                </option>
                {slots.map((slot) => (
          ...
          {date && !slotLoading && slots.length === 0 && (
            <p className="notice">No available time slots for this date. Please choose another date.</p>
          )}
        </section>
      ...
      <style jsx>{`
        ...
        .panel {
          ...
          padding: 24px;
          margin-bottom: 18px;
          box-shadow: 0 12px 34px rgba(40, 30, 22, 0.06);
        }
        ...
        .card.active {
          border-color: #a67c3d;
          box-shadow: 0 10px 24px rgba(166, 124, 61, 0.16);
